...
export class ProductCategoryResultDto {
  ...
  static of(categoryCounts: CategoryCountDto[]): ProductCategoryResultDto[] {
    // 실제 데이터가 있는 카테고리들을 Map으로 변환
    const categoryMap = new Map(
      categoryCounts.map((item) => [item.category, item.count]),
    );

    // 모든 카테고리에 대해 ProductCategoryResultDto 생성 (없는 카테고리는 0으로 설정)
    return Object.values(ProductCategory).map(
      (category) =>
        new ProductCategoryResultDto(category, categoryMap.get(category) ?? 0),
    );
  }
}
