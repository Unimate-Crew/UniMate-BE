...
export class FindUserSalesResponseDto {
  @ApiProperty({
    description: '판매내역 목록',
    type: [UserSalesItemDto],
  })
  contents: UserSalesItemDto[];
  ...
  constructor(contents: UserSalesItemDto[], hasNext: boolean) {
  ...
  ): FindUserSalesResponseDto {
    const response = new FindUserSalesResponseDto(
      productPostInfoSlice.contents.map(
        (info) =>
          new UserSalesItemDto(
