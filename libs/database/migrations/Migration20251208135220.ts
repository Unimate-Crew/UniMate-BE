...
export class Migration20251208135220 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `alter table \`like\` add \`is_deleted\` tinyint(1) not null default false, add \`updated_at\` datetime not null default CURRENT_TIMESTAMP, add \`deleted_at\` datetime null;`,
    );

    this.addSql(
      `alter table \`notification\` modify \`notification_type\` enum('SALE_ENDED', 'PRICE_CHANGED', 'NEW_CHAT_MESSAGE') not null;`,
    );
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(
      `alter table \`like\` drop column \`is_deleted\`, drop column \`updated_at\`, drop column \`deleted_at\`;`,
    );

    this.addSql(
      `alter table \`notification\` modify \`notification_type\` enum('SALE_ENDED', 'PRICE_CHANGED') not null;`,
    );
