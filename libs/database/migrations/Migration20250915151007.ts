...
export class Migration20250915151007 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`alter table \`conversation_participant\` modify \`status\` enum('JOIN', 'LEFT', 'BLOCK', 'MUTE') not null default 'JOIN';`);
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`alter table \`conversation_participant\` modify \`status\` enum('JOIN', 'PRES', 'BLOCK', 'MUTE') not null default 'JOIN';`);
  }
