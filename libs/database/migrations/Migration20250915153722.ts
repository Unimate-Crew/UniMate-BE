...
export class Migration20250915153722 extends Migration {
  ...
  override async up(): Promise<void> {
    // 1단계: 임시 컬럼 추가
    this.addSql('alter table `conversation_participant` add column `status_new` json null;');
    ...
    this.addSql('alter table `conversation_participant` change `status_new` `status` json not null;');
  }
