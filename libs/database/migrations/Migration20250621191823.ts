...
export class Migration20250621191823 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `alter table \`user\` change \`profile_image_url\` \`profile_image_key\` varchar(255) null;`,
    );
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(
      `alter table \`user\` change \`profile_image_key\` \`profile_image_url\` varchar(255) null;`,
    );
  }
}
