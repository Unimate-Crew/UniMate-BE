...
export class Migration20250926170037 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table \`conversation_participant\` add \`is_blocking_other\` tinyint(1) not null default false, add \`is_muted\` tinyint(1) not null default false;`);
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`alter table \`conversation_participant\` drop column \`is_blocking_other\`, drop column \`is_muted\`;`);

    this.addSql(`alter table \`conversation_participant\` add \`status\` json not null;`);
  }
